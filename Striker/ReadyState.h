...
option(ReadyState)
{
	...
	{
		//My Keeper();
		kickOffPos=Transformation::fieldToRobot(theRobotPose,Vector2f(-4500.f,0.f));
	}
	else if(pNumber==3)
	{
		//MyStriker();
	...
	{
		//Supporter
		kickOffPos=Transformation::fieldToRobot(theRobotPose,Vector2f(-1050.f,-200.f));
	}
  initial_state(stand)
  {
	transition
  ...
  state(walkToKickOffPos)
  {
	  ...
	  {
		LookForward();
			WalkToTarget(Pose2f(10.f,10.f,10.f),Pose2f(kickOffPos.angle(),kickOffPos.x(),kickOffPos.y()));
	  }
  }
